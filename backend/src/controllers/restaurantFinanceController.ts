...
export const getRestaurantEarnings = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const pipeline: any[] = [{ $match: match }, { $lookup: { from: 'users', localField: 'user', foreignField: '_id', as: 'customer' } }, { $unwind: { path: '$customer', preserveNullAndEmptyArrays: true } }, { $addFields: { foodTotal: { $subtract: ['$totalAmount', '$deliveryFee'] }, commission: { $multiply: [{ $subtract: ['$totalAmount', '$deliveryFee'] }, COMMISSION_RATE] }, restaurantEarnings: { $multiply: [{ $subtract: ['$totalAmount', '$deliveryFee'] }, 1 - COMMISSION_RATE] } } }];
    ...
    const [summaryRows, transactionRows] = await Promise.all([
      Order.aggregate([{ $match: { restaurant: restaurant._id, orderStatus: 'delivered', paymentStatus: 'paid' } }, { $addFields: { foodTotal: { $subtract: ['$totalAmount', '$deliveryFee'] } } }, { $group: { _id: null, gross: { $sum: '$foodTotal' }, today: { $sum: { $cond: [{ $gte: ['$actualDeliveryTime', today] }, { $multiply: ['$foodTotal', 1 - COMMISSION_RATE] }, 0] } }, yesterday: { $sum: { $cond: [{ $and: [{ $gte: ['$actualDeliveryTime', new Date(today.getTime() - 86400000)] }, { $lt: ['$actualDeliveryTime', today] }] }, { $multiply: ['$foodTotal', 1 - COMMISSION_RATE] }, 0] } }, weekly: { $sum: { $cond: [{ $gte: ['$actualDeliveryTime', week] }, { $multiply: ['$foodTotal', 1 - COMMISSION_RATE] }, 0] } }, previousWeekly: { $sum: { $cond: [{ $and: [{ $gte: ['$actualDeliveryTime', new Date(week.getTime() - 7 * 86400000)] }, { $lt: ['$actualDeliveryTime', week] }] }, { $multiply: ['$foodTotal', 1 - COMMISSION_RATE] }, 0] } }, monthly: { $sum: { $cond: [{ $gte: ['$actualDeliveryTime', month] }, { $multiply: ['$foodTotal', 1 - COMMISSION_RATE] }, 0] } }, previousMonthly: { $sum: { $cond: [{ $and: [{ $gte: ['$actualDeliveryTime', new Date(month.getFullYear(), month.getMonth() - 1, 1)] }, { $lt: ['$actualDeliveryTime', month] }] }, { $multiply: ['$foodTotal', 1 - COMMISSION_RATE] }, 0] } }, net: { $sum: { $multiply: ['$foodTotal', 1 - COMMISSION_RATE] } }, commission: { $sum: { $multiply: ['$foodTotal', COMMISSION_RATE] } } } }]),
