...
import DeliveryFinancePanel from '../../components/admin/DeliveryFinancePanel';
import DeliveryEarningSettingsCard from '../../components/admin/DeliveryEarningSettingsCard';
...
const DeliveryManagementPage: React.FC = () => {
  ...
  return <div className="space-y-7">
    ...
    <div className="grid sm:grid-cols-2 xl:grid-cols-4 gap-4">{[['Delivery Charges Collected',summary?.deliveryChargesCollected],['Paid to Delivery Men',summary?.totalPaidToDeliveryMen],['Pending Delivery Payments',summary?.pendingDeliveryPayments],['Platform Delivery Margin',summary?.platformDeliveryMargin]].map(([label,value])=><div key={String(label)} className="card p-5"><DollarSign className="text-green-400 mb-2"/><div className="text-2xl font-bold">৳{Number(value||0).toLocaleString()}</div><div className="text-sm text-white/50">{label}</div></div>)}</div>
