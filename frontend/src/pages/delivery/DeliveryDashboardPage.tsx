...
const DeliveryDashboardPage: React.FC = () => {
  ...
  const estimatedEarning=(fee:number)=>{const setting=dashboard?.earningSetting;if(!setting)return 0;if(setting.mode==='fixed')return Math.min(fee,setting.fixedAmount);if(setting.mode==='percentage')return Math.min(fee,fee*setting.percentage/100);return Math.min(fee,setting.baseAmount)};
  ...
  return (
    <DeliveryLayout>
      <div className="space-y-8">
        ...
        <section id="earnings" className="scroll-mt-6"><h2 className="text-2xl font-bold mb-4">Earnings & Wallet</h2><div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">{[['Current Balance',dashboard?.wallet.currentBalance],['Pending Earnings',dashboard?.wallet.pendingEarnings],['Paid Earnings',dashboard?.wallet.paidEarnings],['Total Earnings',dashboard?.wallet.totalEarnings]].map(([label,amount])=><div key={String(label)} className="card p-5"><Wallet className="text-green-400 mb-3"/><p className="text-xl font-bold">৳{Number(amount||0).toLocaleString()}</p><p className="text-sm text-white/50">{label}</p></div>)}</div><div className="grid sm:grid-cols-2 lg:grid-cols-5 gap-4 mt-4">{[['Today', dashboard?.earnings.today], ['This Week', dashboard?.earnings.weekly], ['This Month', dashboard?.earnings.monthly], ['Bonus', dashboard?.earnings.bonus], ['Incentives', dashboard?.earnings.incentives]].map(([label, amount]) => <div key={String(label)} className="card p-5"><DollarSign className="text-primary-400 mb-3" /><p className="text-xl font-bold">৳{Number(amount || 0).toLocaleString()}</p><p className="text-sm text-white/50">{label}</p></div>)}</div><div className="card overflow-x-auto mt-4"><h3 className="font-bold p-5 border-b border-white/5">Settlement History</h3><table className="w-full min-w-[760px] text-left text-sm"><thead className="text-white/40"><tr>{['Settlement','Period','Deliveries','Amount','Method','Status','Paid Date'].map(x=><th key={x} className="px-4 py-3">{x}</th>)}</tr></thead><tbody>{dashboard?.settlements.map(row=><tr key={row._id} className="border-t border-white/5"><td className="px-4 py-4 font-mono">{row.settlementId}</td><td className="px-4 py-4">{new Date(row.periodStart).toLocaleDateString()} – {new Date(row.periodEnd).toLocaleDateString()}</td><td className="px-4 py-4">{row.totalDeliveries}</td><td className="px-4 py-4 font-bold text-green-400">৳{row.finalAmount.toLocaleString()}</td><td className="px-4 py-4 capitalize">{row.paymentMethod?.replace('_',' ')||'—'}</td><td className="px-4 py-4 capitalize">{row.status}</td><td className="px-4 py-4">{row.paymentDate?new Date(row.paymentDate).toLocaleString():'—'}</td></tr>)}</tbody></table>{!dashboard?.settlements.length&&<p className="p-8 text-center text-white/40">No settlements recorded yet.</p>}</div></section>
