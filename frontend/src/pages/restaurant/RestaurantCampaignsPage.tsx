...
const RestaurantCampaignsPage: React.FC = () => {
  ...
  return <div className="space-y-6">
    ...
    {show && <div onMouseDown={() => setShow(false)} className="fixed inset-0 z-[70] bg-black/75 backdrop-blur-sm overflow-y-auto p-4"><form onSubmit={submit} onMouseDown={event => event.stopPropagation()} role="dialog" aria-modal="true" className="card max-w-4xl mx-auto p-6 my-6 space-y-6"><div className="flex justify-between"><div><h2 className="text-2xl font-bold">Create Campaign</h2><p className="text-sm text-white/45">Schedule and target a premium promotion</p></div><button type="button" onClick={() => setShow(false)}><X /></button></div>
